...
void service_down(int client_socket_ds, arg_struct *arg){
  ...
  while (recvMsgSize > 0){
	...
	n = strlen(minibuff);
	sprintf( &(minibuff[n]), "\tElement: %d/%d warning on %d\n=====================\n", ((arg->data).list_item[t]).current_number_element, ((arg->data).list_item[t]).max_element, ((arg->data).list_item[t]).warning_number_element);
	n = strlen(minibuff);
      }
	...
	sleep(1);
      ...
      flag = 0;
      for(t= 0; t < (arg->data).number_item; t++ ){
	if( ((arg->data).list_item[t]).current_number_element <= ((arg->data).list_item[t]).warning_number_element ){
	  flag = 1;
	  break;
	}
      }
      // Check if has some item warning
      if( flag ){
	sprintf( minibuff, "Warning client %d:\n", (arg->data).id_client);
	n = strlen(minibuff);
	for(t= 0; t < (arg->data).number_item; t++ ){
	  if( ((arg->data).list_item[t]).current_number_element <= ((arg->data).list_item[t]).warning_number_element ){
	    sprintf( &(minibuff[n]), "\tName item: %s\n", ((arg->data).list_item[t]).name);
