...
int main(int argc, char *argv[])
{
    ...
    // Load data
    if( NULL == (private_data = fopen("config_server.dat", "r"))) {
...
//        printf("2) List all warning client.\n");
        printf("2) Infomation a client.\n");
        printf("q) Quit\n\nChoice: ");
        scanf("%c", &choice);
        while( getchar() != '\n');
...
//                    /* Connect to client and send warning command*/
...
//        }
    ...
    // Watting, when number of thread is 0 => Shutdown server.
    /*for(i = 0; i < (((data_server*)segptr)->core_data).current_number_client; i++){
