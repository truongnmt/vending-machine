...
void config_module(){
  ...
  while( choice != 'q' ){
	...
	printf("\tWarning on: %d element\n", data.list_item[i].warning_number_element);
      }
	...
	for(i = 0; i < data.number_item; i++){
	  if( !strcmp( name_item, (data.list_item[i]).name)){
	    ...
	    while( getchar() != '\n');	    
	    printf("\tWarning on: ");
	    scanf("%d", &(data.list_item[i].warning_number_element));
	    while( getchar() != '\n');
	...
	for(i = 0; i < data.number_item; i++){
	  if( !strcmp( name_item, (data.list_item[i]).name)){
		...
		(data.list_item[i]).warning_number_element = (data.list_item[i + 1]).warning_number_element;
	      }
	      // Write back to config file
	      if( NULL == (config_f = fopen("config.cfg", "w"))){
		printf("Has some error!!!\nPlease close program and retry!\n\n");
